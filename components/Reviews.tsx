    ...
    return (
      <div className="py-8">
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-emerald-700">Loading reviews...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="py-8">
          ...
          <div className="bg-cream-50 rounded-lg p-6 mb-6">
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div className="text-4xl font-bold text-emerald-900">{stats.averageRating.toFixed(1)}</div>
                <div className="mt-2">{renderStars(Math.round(stats.averageRating), 'md')}</div>
                <div className="text-sm text-emerald-700 mt-2">{stats.totalReviews} reviews</div>
              </div>
                  ...
                  {[5, 4, 3, 2, 1].map((rating) => {
                    ...
                    return (
                      <div key={rating} className="flex items-center gap-3">
                        <span className="text-sm text-emerald-900 w-12">{rating} star</span>
                        ...
                        <span className="text-sm text-emerald-700 w-12 text-right">{count}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-12 bg-cream-50 rounded-lg">
          ...
          <h3 className="text-xl font-semibold text-emerald-900 mb-2">No Reviews Yet</h3>
          <p className="text-emerald-700 mb-4">Be the first to review this plant!</p>
          {!isAuthenticated && (
            <p className="text-sm text-emerald-600">Please log in to write a review</p>
          )}
        </div>
      ) : (
        <div className="space-y-4">
          {reviews.map((review) => (
            <div key={review._id} className="bg-white rounded-lg p-6 border border-emerald-100 shadow-sm">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  {review.title && (
                    <h4 className="font-semibold text-emerald-900 mb-2">{review.title}</h4>
                  )}
                  <p className="text-emerald-800 leading-relaxed">{review.comment}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-6">
              <button
                ...
                disabled={page === 1}
                className="px-4 py-2 bg-white border border-emerald-300 text-emerald-700 rounded-lg hover:bg-emerald-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
...
function ReviewForm({ plantId, onReviewAdded }: { plantId: string; onReviewAdded: () => void }) {
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-emerald-200 mb-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="review-title" className="block text-sm font-semibold text-emerald-900 mb-2">
        ...
        <div>
          <label htmlFor="review-comment" className="block text-sm font-semibold text-emerald-900 mb-2">
            Your Review * <span className="text-sm font-normal text-emerald-600">(min. 10 characters)</span>
          </label>
          <textarea
            id="review-comment"
            value={comment}
          ...
          />
          <div className="text-sm text-emerald-600 mt-1 text-right">
